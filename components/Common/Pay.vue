...
<script setup>
      ...
      if (courseInfo.value.orderNo) {
        ...
        if (process.dev) {
          res = {
            ...
            orderStatus: 1,
            payMessage: '模拟支付二维码数据',
            amount: courseInfo.value.coursePrice
          }
        } else {
  ...
  async function orderQuery(orderNo) {
    ...
    if (process.dev) {
      ...
      if (random > 0.7) {
        res = { ...orderInfo.value, orderStatus: 2 } // 支付成功
      } else {
        res = { ...orderInfo.value, orderStatus: 1 } // 继续等待
      }
    } else {
      res = await courseApi.orderInfoView(orderNo)
    }
