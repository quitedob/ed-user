  ...
  <NuxtLayout name="account">
    <el-table v-loading="page.loading" :border="false" :data="page.list" :show-header="false">
      ...
      <el-table-column :width="200" align="center" label="操作">
        <template #default="scope">
          <nuxt-link :to="`/course/${scope.row.courseResp?.id}`" link plain type="primary">
...
<script setup>
  ...
  const mockCollectPage = async (params) => {
    const courses = await getAuthCourses()
    return {
      data: {
        list: courses.slice(0, 5), // 模拟收藏的课程
        totalCount: 5,
        pageCurrent: 1,
        pageSize: 10
      }
